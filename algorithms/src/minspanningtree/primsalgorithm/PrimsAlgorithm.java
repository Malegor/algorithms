...
public class PrimsAlgorithm {
	...
	private Edge findEdge(final Set<Node> done, final Node currentNode) {
		for (final Edge edge : currentNode.getNeigbourEdges())
			if ((done.contains(edge.getStartNode()) || done.contains(edge
					.getEndNode()))
	...
	private void updateNeighbours(final Node currentNode,
			final TreeSet<Node> candidates, final Set<Node> done) {
		Node neighbour;
		for (final Edge edge : currentNode.getNeigbourEdges()) {
			neighbour = edge.getStartNode().equals(currentNode) ? edge
					.getEndNode() : edge.getStartNode();
			if (!done.contains(neighbour)) {
				final int minCost = neighbour.getMinIncomingCost();
				if (edge.getCost() < minCost) {
					// TODO
					// Fazer essa operaçao de forma melhor (aqui é O(n))
